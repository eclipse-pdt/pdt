  ...
  : ^(CLASS_T class_entr_type? IDENTIFIER
  {
   ...
   (^(EXTENDS_T fully_qualified_class_name))? (^(IMPLEMENTS_T fully_qualified_class_name_list))?
      (^(CLASS_BODY class_statement_list))?)
    {
      ...
      if ($fully_qualified_class_name.tree != null) {
          superClassLeft = $fully_qualified_class_name.tree.startIndex;
          superClassRight = $fully_qualified_class_name.tree.endIndex + 1;
      ...
      if ($fully_qualified_class_name_list.tree != null) {
          token = (CommonToken)$fully_qualified_class_name_list.tree.token;
          if (token != null) {
	          interfacesLeft = token.getStartIndex();
	          interfacesRight = token.getStopIndex() + 1;
	        }
      }
  ...
  | ^(INTERFACE_T IDENTIFIER (^(EXTENDS_T fully_qualified_class_name_list))?
    { 
       ...
       if ($fully_qualified_class_name_list.tree != null) {
          ...
          token = (CommonToken)$fully_qualified_class_name_list.tree.token;
          if (token != null) {
            listStartIndex = token.getStartIndex();
            listEndIndex = token.getStopIndex();
          }
       }          
...
@init {
  ...
  : ^(FIELD_DECL {inClassStatementList = true;} variable_modifiers static_var_element+ {inClassStatementList = false;})
  {
    ...
    token = (CommonToken)$variable_modifiers.tree.token;
    int modifierLeft = 0;
    if (token != null) token.getStartIndex();
  ...
  | ^(METHOD_DECL modifier REF_T? IDENTIFIER
    ...
    ({
     ...
     block
     {
        ...
        endIndex = $block.tree.endIndex + 1;
    ...
    | ERROR_TOKEN
		  {
		    ...
		    endIndex = $ERROR_TOKEN.endIndex + 1;
		    int type = $ERROR_TOKEN.specificType;
		    Statement stat = new ASTError(startIndex, endIndex);
        ...
        methodDeclaration.getBody().setEnd(endIndex);
        methodDeclaration.getBody().getStatements().clear();
        methodDeclaration.getBody().acceptStatements(errorList);
		  }
    ))  
    ...
    while (iter.hasNext()) {
      ASTNode[] decl = (ASTNode[])iter.next();
	    if (decl != null) {
		    ...
		    Expression initializer = (Expression)decl[1];
		    int decListLeft = 0;
//		    token = (CommonToken)$directive.tree.token;
//		    if (token != null) decListLeft = token.getStartIndex();
        if ($directive.start != null) {
          decListLeft = $directive.start.startIndex;
        }
  ...
  {     
      ...
      List errorList = new LinkedList();
      errorList.add(stat);
      Block block = new Block(startIndex, endIndex, errorList);
      PHPDocBlock docBlock = null;
      PHPMethodDeclaration methodDeclaration = new PHPMethodDeclaration(startIndex, endIndex, startIndex, endIndex, $ERROR_TOKEN.errorMsg, Modifiers.AccPublic, null, block, false, docBlock);
      parser.addDeclarationStatement(methodDeclaration);
  }
  ;
...
statement returns [Statement stat]
  : ^(STATEMENT topStatement)
  {
    if ($topStatement.expr != null) {
      int startIndex = $topStatement.start.startIndex;
      int endIndex = $topStatement.start.endIndex + 1;
...
@init {
  ...
  | ^(BREAK_T e=expression?)
  {
    int startIndex = $BREAK_T.startIndex;
    int endIndex = $BREAK_T.endIndex + 1;
    if ($e.expr != null) {
      $stat = new BreakStatement(startIndex, endIndex, $e.expr);
    }
    else {
  ...
  | ^(DECLARE_T directive declare_statement)
  {
	    ...
	    $stat = new DeclareStatement(startIndex, endIndex, $topStatement::declareKey, $topStatement::declareValue, $declare_statement.block);
  }
  | ^(TRY_T block catch_branch_list)
  {
    ...
    $stat = new TryStatement(startIndex, endIndex, (Block)$block.stat, $catch_branch_list.catchList);
  }
...
@init {
  ...
  : ^(domain=DOMAIN_T fname=fully_qualified_class_name IDENTIFIER) DOMAIN_T?
  {
    ...
    if ($fname.name != null) {
      ...
      int typeLeft = 1000, typeRight = 0;
      if ($fname.start != null) {
          typeLeft = $fname.start.startIndex;
          typeRight = $fname.start.endIndex + 1;
      }
  ...
  | IDENTIFIER DOMAIN_T?
  {
    ...
    ((SLAST)retval.start).setIndex(typeLeft, typeRight - 1);
  }
  ;
...
@init {
  ...
  : ^(IF_T  ^(CONDITION expression)
    ...
    ))
    {
      ...
      int sid = 0, eid = 0;
      if ($inner_statement_list.start != null) {
        sid = $inner_statement_list.start.startIndex;
        eid = $inner_statement_list.start.endIndex + 1;
      }
...
//  }
  ...
  | ^(NEW_T class_name_reference)
  {
    ...
    $expr = new ClassInstanceCreation(startIndex, endIndex, className, ctor);
  }
...
//     }
     if ($base_variable_with_function_calls.simOrVar == 0) {
        $var = $base_variable_with_function_calls.type;
     }
...
variable returns [Expression var, PHPCallArgumentsList parameterList, Statement errorStat]
scope {
  List list;
}
@init {
...
variable_temp returns [Expression var, PHPCallArgumentsList parameterList, Statement errorStat]
  : base_variable_with_function_calls 
  {
     $var = $base_variable_with_function_calls.var;
     $parameterList = $base_variable_with_function_calls.parameterList;
...
base_variable_with_function_calls returns [Expression var, PHPCallArgumentsList parameterList, TypeReference type, Expression functionInvocation, int simOrVar, Statement errorStat]
  : ^(VAR_DECL fully_qualified_class_name? obj=object_property ctor_arguments?
  {
    $simOrVar = 1; //0: simple; 1: var
    ...
    if ($ctor_arguments.argumentList != null) {
      $parameterList = $ctor_arguments.argumentList;
//      $var = new ReflectionCallExpression(startIndex, endIndex, null, $obj.expr, $parameterList);
//      $var = new ReflectionStaticMethodInvocation(startIndex, endIndex, type, $obj.expr, $parameterList);
    }
//    else {
...
//    }
  ...
  | ^(CALL fully_qualified_class_name ctor_arguments)
  {
    ...
    if ($fully_qualified_class_name.simpleRef != null) {
      $var = new StaticMethodInvocation(startIndex, endIndex, (TypeReference)$fully_qualified_class_name.type, $fully_qualified_class_name.simpleRef, $parameterList);
    }
    else {
      int functionNameLeft = 0, functionNameRight = 0;
      if ($fully_qualified_class_name.start != null) {
         functionNameLeft = $fully_qualified_class_name.start.startIndex;
         functionNameRight = $fully_qualified_class_name.start.endIndex + 1;
      }
      ...
      $var = new PHPCallExpression(startIndex, endIndex, null, name, $parameterList);
    }
...
error_token returns [Expression errorExpr, Statement errorStat]
  :  ^(ERROR_TOKEN variable?)
  {
    ...
    switch(type) {
      case FOR_T:
        $errorStat = new ForStatement(startIndex, endIndex, new LinkedList(), new LinkedList(), new LinkedList(), new Block());
        break;
      case WHILE_T:
        $errorStat = new WhileStatement(startIndex, endIndex, null, new Block());
        break;
      case DO_T:
        $errorStat = new DoStatement(startIndex, endIndex, null, null);
        break;
      case RETURN_T:
        $errorStat = new ReturnStatement(startIndex, endIndex);
        break;
      case GLOBAL_T:
        $errorStat = new GlobalStatement(startIndex, endIndex, new LinkedList());
        break;
      case STATIC_T:
        $errorStat = new StaticStatement(startIndex, endIndex, new LinkedList());
        break;
      case ECHO_T:
        $errorStat = new EchoStatement(startIndex, endIndex, new LinkedList());
        break;
      case FOREACH_T:
        $errorStat = new ForEachStatement(startIndex, endIndex, null, null, new Block());
        break;
      case DECLARE_T:
        $errorStat = new DeclareStatement(startIndex, endIndex, null, null, new Block());
        break;
      case TRY_T:
        $errorStat = new TryStatement(startIndex, endIndex, new Block(), new LinkedList());
        break;
      case THROW_T:
        $errorStat = new ThrowStatement(startIndex, endIndex, null);
        break;
      case USE_T:
        $errorExpr = new PHPCallExpression(startIndex, endIndex, null, "", new PHPCallArgumentsList(0, 0));
        break;
      case INCLUDE_T:
        $errorStat = new Include(startIndex, endIndex, null, Include.IT_INCLUDE);
        break;
      case INCLUDE_ONCE_T:
        $errorStat = new Include(startIndex, endIndex, null, Include.IT_INCLUDE_ONCE);
        break;
      case REQUIRE_T:
        $errorStat = new Include(startIndex, endIndex, null, Include.IT_REQUIRE);
        break;
      case REQUIRE_ONCE_T:
        $errorStat = new Include(startIndex, endIndex, null, Include.IT_REQUIRE_ONCE);
        break; 
      default:
        $errorStat = new ASTError(startIndex, endIndex);
    }
  }
  ;
...
object_property returns [String str, Expression expr, SimpleReference simpleRef, int simOrVar]
@init {
  int startIndex = -1;
  int endIndex = -1;
}
  : ^(VAR var_atom)
  {
    $str = $var_atom.str;
    $expr = $var_atom.expr;
    $simpleRef = $var_atom.simpleRef;
    $simOrVar = $var_atom.simOrVar;   
  }
  ...
  | ^(HASH_INDEX obj=object_property expression?)
  {
    ...
    if (startIndex == 0 && $expression.text != null) {
       startIndex = $obj.start.startIndex;
       endIndex = $obj.start.endIndex + $expression.text.length();
    }
    Expression varName = $obj.expr;
    Expression index = $expression.expr;
    if ($expression.text != null) {
...
var_atom returns [String str, Expression expr, SimpleReference simpleRef, int simOrVar]
@init {
  int startIndex = -1;
  int endIndex = -1;
  SLAST ast = null;
  $simOrVar = 0;  //0: sim; 1: var; 2: other
}
@after {
  ...
  : ^(VAR_ATOM dollars IDENTIFIER)
  {
    ...
    $expr = new VariableReference(startIndex, endIndex, $str ,PHPVariableKind.LOCAL);
  ...
  | ^(ERROR_TOKEN variable?) 
  {
    startIndex = $ERROR_TOKEN.startIndex;
    endIndex = $ERROR_TOKEN.endIndex + 1;
    $expr = new VariableReference(startIndex, endIndex, "" ,PHPVariableKind.LOCAL);
  }
  ;
...
//  }
  ...
  | ^(ERROR_TOKEN variable?)
  {
    ...
    int endIndex = $ERROR_TOKEN.endIndex;
    int type = $ERROR_TOKEN.specificType;
    $stat = new ASTError(startIndex, endIndex);
  }
  ;  
