...
public class PHPModelPresentation extends LabelProvider implements IDebugModelPresentation, IDebugEditorPresentation {
	 ...
	 */
	public static class ExceptionAnnotation extends Annotation {

		private static final String ANNOTATION_TYPE = "org.eclipse.php.debug.ui.phpExceptionAnnotation"; //$NON-NLS-1$
		private PHPExceptionBreakpoint breakpoint;

		public ExceptionAnnotation(PHPExceptionBreakpoint breakpoint) {
			super(ANNOTATION_TYPE, false, breakpoint.getName());
		 ...
		 * @return breakpoint
		 */
		public PHPExceptionBreakpoint getBreakpoint() {
			return breakpoint;
		}
	...
	public boolean addAnnotations(IEditorPart editorPart, IStackFrame frame) {
		...
		for (IBreakpoint breakpoint : breakpoints) {
			if (breakpoint instanceof PHPExceptionBreakpoint) {
				PHPExceptionBreakpoint exceptionBreakpoint = (PHPExceptionBreakpoint) breakpoint;
				try {
	...
	protected String getThreadText(IThread thread) {
		...
		if (thread.isStepping()) {
			...
			if (breakpoints.length == 0) {
				...
				} else if (breakpoint instanceof PHPExceptionBreakpoint) {
	...
	private void addExceptionAnnotation(IEditorPart editorPart, IStackFrame frame, PHPExceptionBreakpoint breakpoint) {
		ITextEditor textEditor = editorPart.getAdapter(ITextEditor.class);
		IDocumentProvider docProvider = textEditor.getDocumentProvider();
		IEditorInput editorInput = textEditor.getEditorInput();
		// If there is no annotation model, there's nothing more to do
		...
		annotation = new ExceptionAnnotation(breakpoint);
