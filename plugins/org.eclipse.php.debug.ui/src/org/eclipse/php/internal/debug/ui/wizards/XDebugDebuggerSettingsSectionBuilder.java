...
public class XDebugDebuggerSettingsSectionBuilder implements
	 ...
	 */
	@Override
	public Composite build(CompositeFragment compositeFragment,
			IDebuggerSettingsWorkingCopy settingsWorkingCopy) {
		switch (settingsWorkingCopy.getKind()) {
		case PHP_SERVER:
			return (new XDebugDebuggerServerSettingsSection()).create(
					compositeFragment, settingsWorkingCopy);
		case PHP_EXE:
			return (new XDebugDebuggerExeSettingsSection()).create(
					compositeFragment, settingsWorkingCopy);
		default:
			break;
		}
