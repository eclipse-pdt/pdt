	...
	public IMethodBinding[] getImportedMethods() {
		...
		if (importedMethods == null) {
			if (isClass() || isTrait()) {
				...
				for (IModelElement element : this.elements) {
					...
					try {
						...
						for (IMethod method : type.getMethods()) {
							nameSet.add(method.getElementName());
						}
						IMethod[] methods = TraitUtils.getTraitMethods(type, nameSet);
						if (methods != null) {
							for (int i = 0; i < methods.length; i++) {
								IMethodBinding methodBinding = resolver.getMethodBinding(methods[i]);
								methodBindings.add(methodBinding);
				...
				importedMethods = methodBindings.toArray(new IMethodBinding[methodBindings.size()]);
			} else {
				importedMethods = new IMethodBinding[0]; // TODO - Implement
				// IMethodBinding
			}
		}
