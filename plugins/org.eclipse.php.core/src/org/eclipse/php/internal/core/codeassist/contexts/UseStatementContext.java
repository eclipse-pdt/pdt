...
public abstract class UseStatementContext extends StatementContext {
	...
	/**
	 * Like getPrefix(), but if it's a grouped use statement, we only handle
	 * statement part before '{'.
	 * 
	 * @return
	 */
	@SuppressWarnings("null")
	...
	/**
	 * Prefix part before the cursor (leading '\' excluded). When
	 * isCursorInsideGroupStatement() is true, only the prefix part inside the
	 * group statement is returned (leading '\' excluded).
	 * 
	 * @return prefix part before the cursor, leading '\' excluded
	 */
	@SuppressWarnings("null")
