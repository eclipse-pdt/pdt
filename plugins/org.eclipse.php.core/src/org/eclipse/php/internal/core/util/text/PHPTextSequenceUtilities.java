		...
		if (tRegion != null && tRegion.getType() == PHPRegionContext.PHP_CONTENT) {
			...
			try {
				...
				while (true) {
					...
					if (removeComments) {
						if (commentEnd != -1 && (type == PHPRegionTypes.PHPDOC_COMMENT_START
								|| type == PHPRegionTypes.PHP_COMMENT_START)) {
							comments.add(new Region(phpScriptRegion.getStart() + startTokenRegion.getStart(),
									commentEnd - startTokenRegion.getStart()));
							commentEnd = -1;
						} else if (type == PHPRegionTypes.PHPDOC_COMMENT_END
								|| type == PHPRegionTypes.PHP_COMMENT_END) {
							commentEnd = startTokenRegion.getEnd();
						} else if (type == PHPRegionTypes.PHP_LINE_COMMENT) {
							comments.add(new Region(phpScriptRegion.getStart() + startTokenRegion.getStart(),
