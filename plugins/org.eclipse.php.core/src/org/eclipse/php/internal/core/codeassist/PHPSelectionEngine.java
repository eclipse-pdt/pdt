				...
				if (phpDocTag.getTagKind() == TagKind.INHERITDOC) {
					...
					for (TypeReference typeReference : phpDocTag.getTypeReferences()) {
						if (typeReference.sourceStart() <= offset && typeReference.sourceEnd() >= end) {
							...
							String fullName = typeReference.getName();
							// remove additional end elements like '[]' or '()'
							if (typeReference.sourceEnd() > end) {
								name = name.substring(offset - typeReference.sourceStart(),
										end - typeReference.sourceStart());
								fullName = fullName.substring(0, end - typeReference.sourceStart());
							}
							String[] parts = fullName.split(Pattern.quote(PAAMAYIM_NEKUDOTAIM), 3);
							if (parts.length > 1) {
								if (parts.length == 2 && parts[0].length() > 0 && parts[1].length() > 0) {
									boolean isVariable = parts[1].charAt(0) == '$';
									// to determine if it was the part before
									// "::" that was selected
									boolean isClassPartSelected = typeReference.sourceStart() + parts[0].length()
											+ PAAMAYIM_NEKUDOTAIM.length() < offset;
									if (isClassPartSelected) {
										return filterNS(
