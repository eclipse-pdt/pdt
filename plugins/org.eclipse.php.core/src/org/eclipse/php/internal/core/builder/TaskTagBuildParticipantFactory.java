...
/**
 * This builder firstly finds all todo task tags in phpdocs and secondly creates
			...
			if (moduleDeclaration instanceof PHPModuleDeclaration) {
				try {
					...
					for (PHPDocBlock phpDocBlock : ((PHPModuleDeclaration) moduleDeclaration).getPhpDocBlocks()) {
						...
						checkForTodo(todos, foundTaskTags, phpDocBlock.start(),
								document.get(phpDocBlock.start(), phpDocBlock.end() - phpDocBlock.start()));
						if (foundTaskTags.isEmpty()) {
							phpDocBlock.setTaskTags(null);
						} else {
							phpDocBlock.setTaskTags(foundTaskTags);
