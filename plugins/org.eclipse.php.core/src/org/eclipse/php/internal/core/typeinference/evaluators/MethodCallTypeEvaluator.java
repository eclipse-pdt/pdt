...
public class MethodCallTypeEvaluator extends GoalEvaluator {
	...
	private String getMethodStringArgument(StaticConstantAccess argument, IContext context) {
		...
		if (localClassName.charAt(0) == NamespaceReference.NAMESPACE_SEPARATOR) {
			// \className::class
			return localClassName.length() == 1 ? null : localClassName.substring(1);
		}
		...
		if (localClassName.indexOf(NamespaceReference.NAMESPACE_DELIMITER) == -1) {
			// className::class
			UsePart usePart = ASTUtils.findUseStatementByAlias(moduleDeclaration, localClassName,
					argument.sourceStart());
			if (usePart != null) {
				// className is defined in a "use" statement
				String fullyQualifiedName = usePart.getNamespace().getFullyQualifiedName();
				...
				return fullyQualifiedName.length() == 1 ? null : fullyQualifiedName.substring(1);
			}
		}
		IType currentNamespace = PHPModelUtils.getCurrentNamespace(sourceModule, argument.sourceStart());
		if (currentNamespace == null) {
			return localClassName;
		}
		return new StringBuilder(currentNamespace.getElementName()).append(NamespaceReference.NAMESPACE_SEPARATOR)
				.append(localClassName).toString();
	}
