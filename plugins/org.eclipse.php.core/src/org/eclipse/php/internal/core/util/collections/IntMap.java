...
public interface IntMap {
	 ...
	 * operation). These mappings will replace any mappings that this map had for
	 ...
	 * not support the add or <tt>addAll</tt> operations.
	 * 
	 ...
	 * returned set is a <tt>Map.Entry</tt>. The set is backed by the map, so
	 ...
	 * by the iterator, except through the iterator's own <tt>remove</tt> operation,
	 * or through the <tt>setValue</tt> operation on a map entry returned by the
	 * iterator.
	 * 
	 * @see Map#entrySet()
	 * @since 1.2
	...
	public interface Entry {
		 ...
		 *             in the backing map.
		 * @throws IllegalArgumentException
		 *             if some aspect of this value prevents it from being stored in the
		 *             backing map.
		 * @throws NullPointerException
		 *             the backing map does not permit <tt>null</tt> values, and the
