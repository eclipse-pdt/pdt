...
public class OldPHPCompletionEngine extends ScriptCompletionEngine {
	...
	protected void calcCompletionOption(IStructuredDocument document, int offset, ISourceModule sourceModule) throws BadLocationException {
		...
		int startPosition = PHPTextSequenceUtilities.readIdentifierStartIndex(phpVersion, statementText, endPosition, true);
		...
		if (inPHPDoc) {
			...
			if (statementText.charAt(tagStart) == TAG_SIGN) {
				++tagStart;
				int tagEnd = PHPTextSequenceUtilities.readIdentifierEndIndex(phpVersion, statementText, tagStart, false);
			...
			startPosition = PHPTextSequenceUtilities.readIdentifierStartIndex(phpVersion, statementText, endPosition, true);
	...
	protected boolean isInFunctionDeclaration(TextSequence text, int offset) {
		...
		int wordStart = PHPTextSequenceUtilities.readIdentifierStartIndex(phpVersion, text, wordEnd, false);
	...
	protected void showInterfaceList(String prefix, int offset) {
		...
		int mask = CodeAssistUtils.EXCLUDE_CLASSES | CodeAssistUtils.EXCLUDE_INTERFACES;
		if (requestor.isContextInformationMode()) {
			mask |= CodeAssistUtils.EXACT_NAME;
		}
	...
	protected boolean isInCatchStatement(TextSequence text, int offset) {
		...
		int endPosition = PHPTextSequenceUtilities.readIdentifierEndIndex(phpVersion, text, startPosition, false);
