...
public class PHPDocTag extends ASTNode implements PHPDocTagKinds {
	...
	public enum Tag {
		...
		INHERITDOC(PHPDocTagKinds.INHERITDOC, "inheritdoc"); //$NON-NLS-1$
		...
		private Tag(int tagKind, String name) {
			...
			this.name = name;
			Mapping.map.put(tagKind, this.name);
		}
		...
		public static String getTagValue(int tagKind) {
			return Mapping.map.get(tagKind);
		}
	}
	...
	private void updateReferences(int start, int end) {
		...
		int valueStart = start + getMatchedTag().length();
	...
	public static String getTagKind(int kind) {
		String name = Tag.getTagValue(kind);
		if (name != null) {
			return name;
		}
		return ERROR;
