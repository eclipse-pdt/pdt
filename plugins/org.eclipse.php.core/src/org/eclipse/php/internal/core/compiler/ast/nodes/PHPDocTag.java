...
public class PHPDocTag extends ASTNode implements PHPDocTagKinds {
	...
	private void updateReferences(int start, int end) {
		...
		if (tagKind == RETURN || tagKind == VAR || tagKind == THROWS
			...
			// XXX: don't allow variable name to be AFTER type declarations,
			// as it's not standard and as ASTUtils#parseVarComment(String
			// content, int start, int end) doesn't support it:
			//
