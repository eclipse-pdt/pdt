...
public class PHPDocTag extends ASTNode implements PHPDocTagKinds {
	 ...
	 * Never null.
	 */
	public String getRawDescText() {
		return rawDescText;
	}

	private String getRawDescText(int wordsToSkip) {
		String text = value.trim();
		if (wordsToSkip == 0) {
			return text;
		}
		String[] split = MagicMemberUtil.WHITESPACE_SEPERATOR.split(value);
	...
	private void updateReferences(int start, int end) {
		...
		rawDescText = getRawDescText(wordsToSkip);
	}
	 ...
	 */
	// XXX: we should remove this method in the future
	// and use getVariableReference()
