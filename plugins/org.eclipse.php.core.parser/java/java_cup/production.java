		...
		if (emit.lr_values()) {
			if (!emit.locations()) {
				...
				ret = "\t\tLocation " + labelname + "xleft = ((java_cup.runtime.ComplexSymbolFactory.ComplexSymbol)"
						...
						((offset == 0) ? ".peek()" : (".elementAt(" + emit.pre("top") + "-" + offset + ")"))
						+ ").xleft;\n" + "\t\tLocation " + labelname
						+ "xright = ((java_cup.runtime.ComplexSymbolFactory.ComplexSymbol)" + emit.pre("stack") +
						// TUM 20050917
						((offset == 0) ? ".peek()" : (".elementAt(" + emit.pre("top") + "-" + offset + ")"))
						+ ").xright;\n";
			}
			...
			if (!rhs[pos].is_action()) {
				...
				if ((label = part.label()) != null || emit._xmlactions) {
					...
					declaration = declaration
							+ make_declaration(label, part.the_symbol().stack_type(), rhs_len - pos - 1);
				}
			}
		}
		return declaration;
	...
	protected void remove_embedded_actions(
		...
		for (int act_loc = 0; act_loc < rhs_length(); act_loc++) {
			if (rhs(act_loc).is_action()) {
				...
				new_nt = non_terminal.create_new(null, lhs().the_symbol().stack_type()); // TUM 20060608 embedded
																							// actions patch
