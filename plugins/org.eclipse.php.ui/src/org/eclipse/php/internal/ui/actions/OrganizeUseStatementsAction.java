...
public class OrganizeUseStatementsAction
	...
	public void run(IAction action) {
		...
		if (input instanceof PHPStructuredEditor) {
			...
			sortUseStatements(moduleDeclaration, doc);
		}
	}

	/**
	 * Sort the blocks of use statements
	 */
	private void sortUseStatements(ModuleDeclaration moduleDeclaration,
			IDocument doc) {
		UseStatement[] statements = ASTUtils.getUseStatements(moduleDeclaration,
				doc.getLength());
		int start = 0, offset = 0;
		String total = doc.get()
				.substring(statements[statements.length - 1].sourceEnd());
		...
		while (start < statements.length) {
			...
			for (int i = start; i < statements.length - 1; i++) {
				try {
					if (doc.getLineOfOffset(statements[i + 1].sourceStart()
							- offset) > doc.getLineOfOffset(
									statements[i].sourceStart() - offset) + 1) {
						last_item = i;
						break;
					}
			...
			List<UseStatement> firstBlock = Arrays.asList(
					Arrays.copyOfRange(statements, start, last_item + 1));
			List<UseStatement> sorted = DocumentUtils.filterAndSort(firstBlock,
					total);

			int length = statements[last_item].sourceEnd()
					- statements[start].sourceStart();
			String newNamespaces = DocumentUtils
					.createStringFromUseStatement(sorted);

			try {
				doc.replace(statements[start].sourceStart() - offset, length,
						newNamespaces);
			} catch (BadLocationException e) {
			}
