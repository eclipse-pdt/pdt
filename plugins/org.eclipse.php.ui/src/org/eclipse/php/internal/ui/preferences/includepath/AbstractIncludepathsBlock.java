	...
	public static void createProject(IProject project, URI locationURI, IProgressMonitor monitor) throws CoreException {
		...
		try {
			if (!project.exists()) {
				...
				project.create(desc, subMonitor.newChild(50));
			}
			if (!project.isOpen()) {
		...
		try {
			...
			for (Iterator iter = buildpathEntries.iterator(); iter.hasNext();) {
				...
				if (res instanceof IFolder && entry.getLinkTarget() == null && !res.exists()) {
					CoreUtility.createFolder((IFolder) res, true, true, subMonitor.newChild(1));
				} else {
					subMonitor.worked(1);
				}
				// 3 ticks
					...
					if (orginalPath == null) {
						if (!folder.exists()) {
							...
							if (entry.getLinkTarget() == null) {
								CoreUtility.createFolder(folder, true, true, subMonitor.newChild(2));
							} else {
							...
							if (!folder.exists()) {
								...
								if (parentPath.segmentCount() > 0) {
									...
									if (!parentFolder.exists()) {
										CoreUtility.createFolder(parentFolder, true, true, subMonitor.newChild(1));
									} else {
										subMonitor.worked(1);
									}
								} else {
								...
								orginalFolder.move(entry.getPath(), true, true, subMonitor.newChild(1));
							}
						} else {
							if (!folder.exists() || !entry.getLinkTarget().equals(entry.getOrginalLinkTarget())) {
								orginalFolder.delete(true, subMonitor.newChild(1));
								folder.createLink(entry.getLinkTarget(), IResource.ALLOW_MISSING_LOCAL,
			...
			phpProject.setRawBuildpath(buildpath, subMonitor.newChild(2));
		} finally {
			subMonitor.done();
		}
	}
