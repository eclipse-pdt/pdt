...
public final class ScriptDeleteProcessor extends DeleteProcessor implements ICommentProvider {
	...
	private void addDeletableParentPackagesOnPackageDeletion() throws CoreException {
		...
		final Set/* <IResource> */<IResource> deletedChildren = new HashSet<>();
		...
		final List/* <IScriptFolder */<IScriptFolder> allFragmentsToDelete = new ArrayList<>();
		...
		for (Iterator<?> outerIter = initialPackagesToDelete.iterator(); outerIter.hasNext();) {
			...
			if (canRemoveCompletely(currentScriptFolder, initialPackagesToDelete)) {
				...
				if (parent != null && !initialPackagesToDelete.contains(parent)) {
					...
					final List/* <IScriptFolder> */<IScriptFolder> emptyParents = new ArrayList<>();
		...
		final List/* <IModelElement> */<IModelElement> modelElements = new ArrayList<>();
		for (int i = 0; i < fScriptElements.length; i++) {
			if (!(fScriptElements[i] instanceof IScriptFolder)) {
		...
		final List/* <IResource> */<IResource> resources = new ArrayList<>();
		for (int i = 0; i < fResources.length; i++) {
	 ...
	 * 
	 */
	private void addDeletableParentPackages(IScriptFolder frag, List<?> initialPackagesToDelete, Set<IResource> resourcesToDelete,
			List<IScriptFolder> deletableParentPackages) throws CoreException {
