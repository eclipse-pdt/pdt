...
public class DeleteSourceManipulationChange extends AbstractDeleteChange {
	...
	protected Change doDelete(IProgressMonitor pm) throws CoreException {
		...
		if (element instanceof ISourceModule) {
			...
			saveCUnitIfNeeded(unit, subMonitor.newChild(1));
			...
			if (resource != null) {
				...
				element.delete(false, subMonitor.newChild(1));
				resourceDescription.recordStateFromHistory(resource, subMonitor.newChild(1));
				return new UndoDeleteResourceChange(resourceDescription);
			} else {
				element.delete(false, subMonitor.newChild(3));
			}
			subMonitor.done();
			return null;
