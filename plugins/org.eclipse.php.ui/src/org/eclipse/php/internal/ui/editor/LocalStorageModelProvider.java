		...
		try {
			...
			if (storage != null) {
				...
				if(storage instanceof ExternalSourceModule){
					...
					//each phar file contains a /.phar/stub.php file
					//when one of them is opened in an editor
					//try to open another stub.php will fail with an exception
					//storage.getFullPath() of all the stub.php files are same
					//so this method(calculateID) returns the same result
					//so i add a prefix to the storagePath
					if(externalSourceModule.getParent() instanceof ArchiveFolder){
						storagePath = externalSourceModule.getParent().getPath().append(storagePath);
					}
				}
				String name = storage.getName();
