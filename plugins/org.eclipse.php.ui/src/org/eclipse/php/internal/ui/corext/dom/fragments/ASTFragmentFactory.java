...
public class ASTFragmentFactory {
	...
	private static boolean isSingleNodeSelected(SelectionAnalyzer sa, SourceRange range, IDocument document, ASTNode scope) throws BadLocationException  {
		return sa.getSelectedNodes().length == 1 && !rangeIncludesNonWhitespaceOutsideNode(range, sa.getFirstSelectedNode(), document, scope);
	}
	 ...
	 * the node to which the produced IASTFragment maps (see {@link  IASTFragment  IASTFragment})
