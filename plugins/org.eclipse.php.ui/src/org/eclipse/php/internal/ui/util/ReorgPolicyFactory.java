	...
	private static class MoveFilesFoldersAndCusPolicy extends FilesFoldersAndCusReorgPolicy implements IMovePolicy {
		...
		private Change createReferenceUpdatingMoveChange(IProgressMonitor pm) throws ModelException {
			...
			try {
				...
				if (fChangeManager == null) {
					fChangeManager = createChangeManager(subMonitor.newChild(1), new RefactoringStatus()); // TODO:
																											// non-CU
																											// matches
					// silently dropped
				...
				Change fileMove = createSimpleMoveChange(subMonitor.newChild(1));
		...
		private void computeQualifiedNameMatches(IProgressMonitor pm) throws ModelException {
			...
			if (destination != null) {
				...
				for (int i = 0; i < cus.length; i++) {
					...
					SubMonitor typesMonitor = SubMonitor.convert(subMonitor.newChild(1), types.length);
					for (int j = 0; j < types.length; j++) {
						handleType(types[j], destination, typesMonitor.newChild(1));
		...
		public RefactoringStatus checkFinalConditions(IProgressMonitor pm, CheckConditionsContext context,
			...
			try {
				...
				confirmMovingReadOnly(reorgQueries);
				fChangeManager = createChangeManager(subMonitor.newChild(2), result);
				if (fUpdateQualifiedNames) {
					computeQualifiedNameMatches(subMonitor.newChild(4));
				}
				result.merge(super.checkFinalConditions(subMonitor.newChild(1), context, reorgQueries));
				return result;
			} catch (ModelException e) {
				throw e;
			} catch (CoreException e) {
