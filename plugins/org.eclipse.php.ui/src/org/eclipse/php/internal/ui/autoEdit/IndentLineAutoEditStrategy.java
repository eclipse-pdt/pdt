...
public class IndentLineAutoEditStrategy extends DefaultIndentationStrategy
	...
	private void autoIndentAfterNewLine(final IStructuredDocument document,
		...
		try {
			...
			final IRegion firstLineInfo = document
					.getLineInformationOfOffset(startOffset);
			final int firstLineOffset = firstLineInfo.getOffset();
			final int firstLineLength = firstLineInfo.getLength();
			final String firstLineText = document.get(firstLineOffset,
					firstLineLength);

			// In most case, selections are empty or single-line selections,
			// so do some optimizations.
			final boolean isOnSameLine = firstLineOffset <= endOffset
					&& endOffset <= firstLineOffset + firstLineLength;

			final IRegion lastLineInfo = isOnSameLine ? firstLineInfo
					: document.getLineInformationOfOffset(endOffset);
			final int lastLineOffset = isOnSameLine ? firstLineOffset
					: lastLineInfo.getOffset();
			final int lastLineLength = isOnSameLine ? firstLineLength
					: lastLineInfo.getLength();
			final String lastLineText = isOnSameLine ? firstLineText : document
					.get(lastLineOffset, lastLineLength);
