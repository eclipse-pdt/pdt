...
public class BindingLabelProvider extends LabelProvider {
	...
	private static void getFieldLabel(IVariableBinding binding, long flags, StringBuffer buffer) {
		if (((flags & ScriptElementLabels.F_PRE_TYPE_SIGNATURE) != 0)/* && !binding.isEnumConstant()*/) {
		...
		if (((flags & ScriptElementLabels.F_APP_TYPE_SIGNATURE) != 0)/* && !binding.isEnumConstant()*/) {
	...
	private static void getMethodLabel(IFunctionBinding binding, long flags, StringBuffer buffer) {
		...
		if ((flags & ScriptElementLabels.M_FULLY_QUALIFIED) != 0 && methodBinding != null) {
			getTypeLabel(methodBinding.getDeclaringClass(), ScriptElementLabels.T_FULLY_QUALIFIED | (flags & ScriptElementLabels.P_COMPRESSED), buffer);
			buffer.append('.');
		}
		buffer.append(binding.getName());
	...
	/**
	 * Returns the image descriptor for a binding with the flags as defined by {@link ScriptElementImageProvider}.
	 * @param binding The binding to get the image for.
	 * @param imageFlags The image flags as defined in {@link ScriptElementImageProvider}.
	...
	/**
	 * @param textFlags Flags defined in {@link ScriptElementLabels}.
