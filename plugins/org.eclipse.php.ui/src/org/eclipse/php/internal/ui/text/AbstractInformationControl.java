...
public abstract class AbstractInformationControl extends PopupDialog implements IInformationControl, IInformationControlExtension, IInformationControlExtension2, DisposeListener {
	...
	private PHPCodeData findElement(TreeItem[] items) {
		ILabelProvider labelProvider = (ILabelProvider) fTreeViewer.getLabelProvider();
		for (int i = 0; i < items.length; i++) {
			PHPCodeData element = (PHPCodeData) items[i].getData();
	...
	protected IDialogSettings getDialogSettings() {
		...
		IDialogSettings settings = PHPUiPlugin.getDefault().getDialogSettings().getSection(sectionName);
		if (settings == null)
			settings = PHPUiPlugin.getDefault().getDialogSettings().addNewSection(sectionName);
