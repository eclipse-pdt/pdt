...
class FoldingConfigurationBlock implements IPreferenceConfigurationBlock {
	...
	public Control createControl(Composite parent) {
		...
		fFoldingCheckbox.setText(PHPUIMessages.getString("FoldingConfigurationBlock_enable"));
		...
		if (fProviderDescriptors.size() > 1) {
			...
			comboLabel.setText(PHPUIMessages.getString("FoldingConfigurationBlock_combo_caption"));
	...
	private ComboViewer createProviderViewer() {
		...
		viewer.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (!sel.isEmpty()) {
					fStore.setValue(PreferenceConstants.EDITOR_FOLDING_PROVIDER, ((PHPFoldingStructureProviderDescriptor) sel.getFirstElement()).getId());
	...
	void updateListDependencies() {
		...
		PHPFoldingStructureProviderDescriptor desc= (PHPFoldingStructureProviderDescriptor) fProviderDescriptors.get(id);
		IPHPFoldingPreferenceBlock prefs;

		if (desc == null) {
			// safety in case there is no such descriptor
			String message= PHPUIMessages.getString("FoldingConfigurationBlock_error_not_exist");
		...
		if (control == null) {
			...
			if (control == null) {
				String message= PHPUIMessages.getString("FoldingConfigurationBlock_info_no_preferences");
