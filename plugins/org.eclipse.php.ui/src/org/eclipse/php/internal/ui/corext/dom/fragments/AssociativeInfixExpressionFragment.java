...
public class AssociativeInfixExpressionFragment extends ASTFragment implements IExpressionFragment {
	...
	private static boolean rangeStartsBetween(SourceRange range, ASTNode first, ASTNode next) {
		...
		return first.getStart() + first.getLength() <= pos && pos <= next.getStart();
	}
	...
	private static boolean rangeIncludesExtraNonWhitespace(SourceRange range, List<Expression> operands, IDocument document, ASTNode scope) throws BadLocationException {
	...
	private static SourceRange getRangeOfOperands(List/*<Expression>*/operands) {
		...
		return new SourceRange(first.getStart(), last.getStart() + last.getLength() - first.getStart());
	}
	...
	private static boolean isOperatorAssociative(int operator) {
		return operator == InfixExpression.OP_PLUS || operator == InfixExpression.OP_MUL || operator == InfixExpression.OP_CONCAT || operator == InfixExpression.OP_XOR || operator == InfixExpression.OP_OR || operator == InfixExpression.OP_AND || operator == InfixExpression.OP_STRING_AND
			|| operator == InfixExpression.OP_STRING_OR || operator == InfixExpression.OP_STRING_XOR || operator == InfixExpression.OP_BOOL_AND || operator == InfixExpression.OP_BOOL_OR;
	}
	...
	private IASTFragment[] getSubFragmentsWithMyNodeMatching(IASTFragment toMatch) {
		...
		List<List> matchingSubsequences = getMatchingContiguousNodeSubsequences(getOperands(), kinToMatch.getOperands());
