...
public class SearchPattern {
	 ...
	 * character.
	 * If third argument is <code>true</code>, then pattern match behaves as
	 * if the given pattern ends with '*', thus, for example, matching 'aabbc'
	 * string with 'aabb' pattern will return a true result.
	 * The match is case sensitive.
	 * 
	 * @param pattern The string pattern
	 * @param str The string to match
	 * @param simulateWildcard simulate a wildcard at the end of the pattern
	...
	public static boolean match(String pattern, String str, boolean simulateWildcard) {
		...
		switch (pattern.charAt(0)) {
			...
			case '*': //'*' we can match 0 letters or more: match the rest of the pattern with str (0 letters match) OR the pattern with the rest of str (1 or more letters match)
				return match(pattern.substring(1), str, simulateWildcard) || match(pattern, str.substring(1), simulateWildcard);
			default: //we have a letter in the pattern: check that the letter match and the rest of the pattern match the rest of str
				return str.charAt(0) == pattern.charAt(0) && match(pattern.substring(1), str.substring(1), simulateWildcard);
		}
	}
	 ...
	 * carachters, and the '?' symbol which can replace any single
	 * character.
	 * If fourth argument is <code>true</code>, then pattern match behaves as
	 * if the given pattern ends with '*', thus, for example, matching 'aabbc'
	 * string with 'aabb' pattern will return a true result.
	 * 
	 * @param pattern The string pattern
	 ...
	 * carachters, and the '?' symbol which can replace any single
	 * character.
	 * The pattern match behaves as if the given pattern ends with '*',
	 * thus, for example, matching 'aabbc' string with 'aabb' pattern will
