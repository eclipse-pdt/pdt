...
public class PHPStructuredEditor extends StructuredTextEditor implements IPhpScriptReconcilingListener {
	...
	private ITaskTagsListener fTaskTagsListener;
	...
	private void initTaskTagsListener() {
		if (fTaskTagsListener != null) {
			return;
		}

		fTaskTagsListener = new ITaskTagsListener() {
			...
			public void taskTagsChanged(TaskTagsEvent event) {
				// Only keep generic TaskTagsEvents and TaskTagsEvents for
				// actual project.
				if (event.getProject() != null && event.getProject() != getProject()) {
					return;
				}
				String newEvent = event.toString();
				// Avoid reparsing a document if same event was sent multiple
				// times. For event comparison, we use the string representation
				// of TaskTagsEvents, it should be enough here. Even so, we will
				// get 2 events: event for previous TaskTag values and event
				// for newest TaskTag values.
				if (StringUtils.equals(oldEvent, newEvent)) {
					return;
				}
				oldEvent = newEvent;
		...
		TaskTagsProvider.getInstance().addTaskTagsListener(fTaskTagsListener);
	}
	...
	public void dispose() {
		...
		if (fTaskTagsListener != null) {
			TaskTagsProvider.getInstance().removeTaskTagsListener(fTaskTagsListener);
			fTaskTagsListener = null;
		}
	...
	protected void doSetInput(IEditorInput input) throws CoreException {
		...
		if (resource != null) {
			if (PHPToolkitUtil.isPhpFile((IFile) resource)) {
				...
				// was moved from one PHP project to another) and
				// TaskTagsProvider project listeners were added for documents
				// (moved or created in new projects) freshly reparsed by
				// PhpStructuredDocumentReParser and PhpSourceParser.
				initPHPVersionsListener();
				initTaskTagsListener();
