...
public class PHPCodeTemplateBlock extends PHPCoreOptionsConfigurationBlock {
	...
	protected final static Object COMMENT_NODE= PHPUIMessages.getString("CodeTemplateBlock_templates_comment_node"); 
	protected final static Object CODE_NODE= PHPUIMessages.getString("CodeTemplateBlock_templates_code_node"); 
...
//		PHPStructuredTextViewer viewer= new PHPStructuredTextViewer(parent, null, null, false, SWT.BORDER | SWT.V_SCROLL | SWT.H_SCROLL);
//		//PHPStructuredTextViewerConfiguration configuration= new PHPStructuredTextViewerConfiguration(PHPUiPlugin.getDefault().getColorManager(), store, null, fTemplateProcessor);
//		//ScriptSourceViewerConfiguration configuration = new PHPStructuredEditor().getSourceViwerConfiguration();
//		
...
//		control.setLayoutData(data);
		...
		StructuredTextViewer viewer = new StructuredTextViewer(parent, null, null, false, SWT.BORDER | SWT.V_SCROLL | SWT.H_SCROLL | SWT.READ_ONLY);
...
//		PHPStructuredTextViewerConfiguration configuration = new PHPStructuredTextViewerConfiguration();
	...
	protected void updateSourceViewerInput(List selection) {
		...
		if (selection.size() == 1 && selection.get(0) instanceof TemplatePersistenceData) {
			...
			TemplateContextType type= PHPUiPlugin.getDefault().getTemplateContextRegistry().getContextType(template.getContextTypeId());
	...
	private void edit(TemplatePersistenceData data) {
		...
		EditTemplateDialog dialog= new EditTemplateDialog(getShell(), newTemplate, true, false, PHPUiPlugin.getDefault().getTemplateContextRegistry());
...
/*		if (file.isHidden()) {
	...
	private boolean confirmOverwrite(File file) {
		...
		return MessageDialog.openQuestion(getShell(),
			PreferencesMessages.CodeTemplateBlock_export_exists_title, "XXXXX"); 
