...
public class PHPDocumentationContentAccess {
	...
	private void handleExceptionTags(List tags, List exceptionNames, CharSequence[] exceptionDescriptions) {
		...
		for (int i = 0; i < exceptionDescriptions.length; i++) {
			...
			if (name != null) {
				stringList.add(handleThrowsException(name, rawDescription));
			}
		}

		// filter duplicates
		List<String> deDupStringList = new ArrayList<>(new HashSet<>(stringList));
		// sort remaining exceptions
		Collections.sort(deDupStringList);

		for (int i = 0; i < deDupStringList.size(); i++) {
			fBuf.append(BlOCK_TAG_ENTRY_START);
			fBuf.append(deDupStringList.get(i));
			doWorkAround();
			fBuf.append(BlOCK_TAG_ENTRY_END);
		}
	}
