...
public class CacheGrindModelParser {
		...
		private ProfilerFunctionData regFunction(ProfilerFileData file, int id, String name) {
			if (id < 0) {
				if (functionIds.containsKey(name)) {
					return dataFunction.get(functionIds.get(name));
				} else {
					id = lastFunctionId--;
					ProfilerFunctionData tmp = new ProfilerFunctionData();
					tmp.setLocalFileName(file.getLocalName());
					tmp.setID(id);
					dataFunction.put(id, tmp);
					functionIds.put(name, id);

					return tmp;
				}
			} else {
				ProfilerFunctionData tmp;
				if (dataFunction.containsKey(id)) {
					tmp = dataFunction.get(id);
				} else {
					tmp = new ProfilerFunctionData();
					tmp.setLocalFileName(file.getLocalName());
					tmp.setID(id);
					dataFunction.put(id, tmp);
				}
	...
	private void finishCurrent() {
		...
		for (Entry<String, Integer> entry : functionIds.entrySet()) {
			ProfilerFunctionData func = dataFunction.get(entry.getValue());
