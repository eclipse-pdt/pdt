...
public class DebugConnection {
				...
				try {
					...
					try {
						...
						if (incomingMessage instanceof DebugSessionStartedNotification) {
							...
							isDebugConnectionTest = sessionStartedMessage.getQuery().indexOf("testConnection=true") != -1; //$NON-NLS-1$
							...
							if (isDebugConnectionTest) {
								...
								if (verifyProtocolID(sessionStartedMessage.getServerProtocolID())) {
									...
									DebugServerTestController.getInstance().notifyTestListener(
											new DebugServerTestEvent(sourceHost,
	...
	public DebugConnection(Socket socket) {
		debugResponseTimeout = PHPProjectPreferences.getModelPreferences().getInt(
	 ...
	 */
	public boolean hookDebugSession(DebugSessionStartedNotification debugSessionStartedNotification)
		...
		if (launch == null) {
			...
			for (ILaunch aLaunch : launchs) {
				...
				if (aLaunch.isTerminated()
						&& (IPHPDebugConstants.DEBUGGING_ALL_PAGES.equals(debugType) || IPHPDebugConstants.DEBUGGING_START_FROM
								.equals(debugType))) {
					launch = aLaunch;
					break;
				}
	 ...
	 */
	protected void hookServerDebug(final ILaunch launch, DebugSessionStartedNotification startedNotification)
			throws CoreException {
	 ...
	 *            A DebugSessionStartedNotification
	 */
	protected void hookPHPExeDebug(final ILaunch launch, DebugSessionStartedNotification startedNotification)
			throws CoreException {
		ILaunchConfiguration launchConfiguration = launch.getLaunchConfiguration();
		messageReceiver.setTransferEncoding(launchConfiguration
				.getAttribute(IDebugParametersKeys.TRANSFER_ENCODING, "")); //$NON-NLS-1$
	...
	protected int getSessionID(String query) {
		int indx = query.lastIndexOf(AbstractDebugParametersInitializer.DEBUG_SESSION_ID + "="); //$NON-NLS-1$
		if (indx < 0) {
			return -1;
		}
