...
public class XDebugConfigurationDialog extends AbstractDebuggerConfigurationDialog {
	...
	private void internalInitializeValues() {
		...
		showGlobals.setSelection(
				Boolean.parseBoolean(service.get(XDebugPreferenceMgr.XDEBUG_PREF_SHOWSUPERGLOBALS, null, null)));
		useMultiSession.setSelection(
				Boolean.parseBoolean(service.get(XDebugPreferenceMgr.XDEBUG_PREF_MULTISESSION, null, null)));
		variableDepth
				.setSelection(Integer.parseInt(service.get(XDebugPreferenceMgr.XDEBUG_PREF_ARRAYDEPTH, null, null)));
		maxChildren.setSelection(Integer.parseInt(service.get(XDebugPreferenceMgr.XDEBUG_PREF_CHILDREN, null, null)));
		maxData.setSelection(Integer.parseInt(service.get(XDebugPreferenceMgr.XDEBUG_PREF_DATA, null, null)));
		acceptRemoteSession
				.select(Integer.parseInt(service.get(XDebugPreferenceMgr.XDEBUG_PREF_REMOTESESSION, null, null)));

		// capture output
		captureStdout.select(Integer.parseInt(service.get(XDebugPreferenceMgr.XDEBUG_PREF_CAPTURESTDOUT, null, null)));
		captureStderr.select(Integer.parseInt(service.get(XDebugPreferenceMgr.XDEBUG_PREF_CAPTURESTDERR, null, null)));

		// proxy defaults
		boolean useProxyState = Boolean.parseBoolean(service.get(XDebugPreferenceMgr.XDEBUG_PREF_USEPROXY, null, null));
		useProxy.setSelection(useProxyState);
		String ideKey = service.get(XDebugPreferenceMgr.XDEBUG_PREF_IDEKEY, null, null);
		if (ideKey == null || ideKey.length() == 0) {
			ideKey = ""; //$NON-NLS-1$
		}
		idekeyTextBox.setText(ideKey);
		proxyTextBox.setText(service.get(XDebugPreferenceMgr.XDEBUG_PREF_PROXY, null, null));
