...
public class DBGpTarget extends DBGpElement
	...
	private String escapeFullName(@NonNull String fullName) {
		...
		int idxLast = idxFirst != -1 ? fullName.lastIndexOf('"') : -1;
		if (idxFirst != idxLast) {
			newVariableName = newVariableName.substring(0, idxFirst)
					+ newVariableName.substring(idxFirst, idxLast).replace("$", "\\\\$") //$NON-NLS-1$ //$NON-NLS-2$
					+ newVariableName.substring(idxLast);
		}
