...
public class CodeDataSearchEngine {
	...
	private static void collectCallbacks(DOMModelForPHP model, BucketMap<String, CodeDataMatch> searchResults,
		...
		Set<String> set = searchElementNames(model, PATTERN_ELEMENT_CALLBACK_DOUBLE, 1, subMonitor.split(1));
		if (monitor.isCanceled())
			return;
		fillSearchResults(searchResults, set, ELEMENT_CALLBACK, true, subMonitor.split(1));
		if (monitor.isCanceled())
			return;
		set = searchElementNames(model, PATTERN_ELEMENT_CALLBACK_SINGLE, 1, subMonitor.split(1));
		if (monitor.isCanceled())
			return;
		fillSearchResults(searchResults, set, ELEMENT_CALLBACK, true, subMonitor.split(1));
		subMonitor.done();
	...
	private static void collectClasses(DOMModelForPHP model, BucketMap<String, CodeDataMatch> searchResults,
		...
		// if (monitor.isCanceled())
		// return;
		collectClassesFromText(model, searchResults, subMonitor.split(1));
		subMonitor.done();
	}
	...
	private static void collectClassesFromText(DOMModelForPHP model, BucketMap<String, CodeDataMatch> searchResults,
		...
		if (monitor.isCanceled())
			return;
		Set<String> set = searchElementNames(model, PATTERN_CLASS_NEW, 1, subMonitor.split(1));
		if (monitor.isCanceled())
			return;
		fillSearchResults(searchResults, set, ELEMENT_CLASS_NEW, subMonitor.split(1));
		if (monitor.isCanceled())
			return;
		set = searchElementNames(model, PATTERN_CLASS_MEMBER, 1, subMonitor.split(1));
		if (monitor.isCanceled())
			return;
		fillSearchResults(searchResults, set, ELEMENT_CLASS_MEMBER, subMonitor.split(1));
		if (monitor.isCanceled())
			return;
		set = searchElementNames(model, PATTERN_CLASS_INSTANCE, 1, subMonitor.split(1));
		if (monitor.isCanceled())
			return;
		fillSearchResults(searchResults, set, ELEMENT_CLASS_INSTANCE, subMonitor.split(1));
		subMonitor.done();
	}
	...
	private static void collectConstants(DOMModelForPHP model, BucketMap<String, CodeDataMatch> searchResults,
		...
		if (monitor.isCanceled())
			return;
		Set<String> set = searchElementNames(model, PATTERN_CONSTANT_READ, 1, subMonitor.split(1));
		if (monitor.isCanceled())
			return;
		fillSearchResults(searchResults, set, ELEMENT_CONSTANT_READ, true, subMonitor.split(1));
		subMonitor.done();
	}
	...
	private static void collectFunctions(DOMModelForPHP model, BucketMap<String, CodeDataMatch> searchResults,
		...
		if (subMonitor.isCanceled())
			return;
		Set<String> set = searchElementNames(model, PATTERN_FUNCTION_CALL, 1, subMonitor.split(1));
		if (subMonitor.isCanceled())
			return;
		fillSearchResults(searchResults, set, ELEMENT_FUNCTION_CALL, subMonitor.split(1));
		subMonitor.done();
	}
	...
	private static void collectInterfaces(DOMModelForPHP model, BucketMap<String, CodeDataMatch> searchResults,
		...
		collectInterfacesFromText(model, searchResults, subMonitor.split(1));
		subMonitor.done();
	}
	...
	private static void collectInterfacesFromText(DOMModelForPHP model, BucketMap<String, CodeDataMatch> searchResults,
		...
		if (monitor.isCanceled())
			return;
		Set<String> set = searchElementNames(model, PATTERN_CLASS_MEMBER, 1, subMonitor.split(1));
		if (monitor.isCanceled())
			return;
		fillSearchResults(searchResults, set, ELEMENT_CLASS_MEMBER, subMonitor.split(1));
		if (monitor.isCanceled())
	...
	// validateCallbacks(DOMModelForPHP model, BucketMap<String, CodeDataMatch>
	...
	//
	// private static DoubleBucketMap<String, CodeDataMatch, CodeData>
	// validateClasses(DOMModelForPHP model, BucketMap<String, CodeDataMatch>
	// searchResults, IProgressMonitor monitor) {
	// BucketMap<String, CodeDataMatch> validSearchResults = new
	// BucketMap<String, CodeDataMatch>();
	...
	//
	// private static DoubleBucketMap<String, CodeDataMatch, CodeData>
	// validateConstants(DOMModelForPHP model, BucketMap<String, CodeDataMatch>
	// searchResults, IProgressMonitor monitor) {
	// BucketMap<String, CodeDataMatch> validSearchResults = new
	// BucketMap<String, CodeDataMatch>();
	...
	//
	// private static DoubleBucketMap<String, CodeDataMatch, CodeData>
	// validateFunctions(DOMModelForPHP model, BucketMap<String, CodeDataMatch>
	// searchResults, IProgressMonitor monitor) {
	// BucketMap<String, CodeDataMatch> validSearchResults = new
	// BucketMap<String, CodeDataMatch>();
	...
	//
	// private static DoubleBucketMap<String, CodeDataMatch, CodeData>
	// validateInterfaces(DOMModelForPHP model, BucketMap<String, CodeDataMatch>
	// searchResults, IProgressMonitor monitor) {
	// BucketMap<String, CodeDataMatch> validSearchResults = new
	// BucketMap<String, CodeDataMatch>();
